...
- [x] **Winston transport drops every record — share one core singleton across subpath bundles** — `ObserveTransport` (`observe-js/winston`) drops all `Logger.log(...)`; in mind_api only `service.start` reaches Loki. Cause: `tsup` builds `node`/`winston` as separate entries with no `splitting`, so `dist/winston.cjs` inlines a duplicate `core/sdk.ts` whose `_batcher` `init()` never touches. Fix: winston self-imports `log` from `'observe-js'` with `external: ['observe-js']` → one shared runtime singleton; drop the vestigial `core` entry (`splitting` rejected: esbuild ESM-only). Guard: no `init()` in the transport; keep `./winston` exports. Regression: cross-bundle built-`dist/` test (`.cjs`+`.mjs`) asserting a real `winston.createLogger` lands a plain `logger.info(...)` at a stubbed `fetch`. Publish to `main`, no tag. Spec: `.ai-factory/notes/14-winston-subpath-shared-core-singleton.md`. [27m 5s]

- [x] **Distribution fix — remove git submodule, tag v0.2.0** — today `contract/` is a git submodule so any `npm install git+https://…` skips `git submodule update --init`, `prepare` fails, and consumers are forced to vendor a pre-built tgz with `prepare` stripped; fix by running `git submodule deinit -f contract && git rm -f contract`, copying `contract/levels.json` as a plain tracked file (import path in `src/core/levels.ts` stays unchanged), keeping `prepare: "npm run build"` as-is, committing, tagging `v0.2.0`, pushing; then updating `mind_api` and `mind_web` to drop `vendor/observe-js-0.0.0.tgz` and point to `#v0.2.0`. Do not commit `dist/`. Spec: `.ai-factory/notes/15-distribution-fix.md`.
